...
public abstract class ValidatorNumberBase implements Validator {
    ...
    private void validateMinimum(final List<Error> errors, final String path, final Object instance,
            final JsonSchema jsonSchema) {
    ...
    private void validateMaximum(final List<Error> errors, final String path, final Object instance,
            final JsonSchema jsonSchema) {
    ...
    private void validateExclusiveMinimum(final List<Error> errors, final String path, final Object instance,
            final JsonSchema jsonSchema) {
        if (!jsonSchema.exclusiveMinimum().isEmpty()) {
            if (compareTo((Number) instance, jsonSchema.exclusiveMinimum()) != 1) {
                errors.add(
                        Error.exclusiveMinimum(path, jsonSchema, getArgument(jsonSchema.exclusiveMinimum()), instance));
            }
        }
    }

    private void validateExclusiveMaximum(final List<Error> errors, final String path, final Object instance,
            final JsonSchema jsonSchema) {
        if (!jsonSchema.exclusiveMaximum().isEmpty()) {
            if (compareTo((Number) instance, jsonSchema.exclusiveMaximum()) != -1) {
                errors.add(
                        Error.exclusiveMaximum(path, jsonSchema, getArgument(jsonSchema.exclusiveMaximum()), instance));
            }
        }
    }

    private void validateMultipleOf(final List<Error> errors, final String path, final Object instance,
            final JsonSchema jsonSchema) {
